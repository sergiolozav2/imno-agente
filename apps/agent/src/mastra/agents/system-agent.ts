...
const BASE_INSTRUCTIONS = [
  ...
  '- When a video reports failed, state the reason from its `detail` field. Never answer with a bare "it failed" or "there was an error": the reason is always there, so pass it on.',
  '- Use the `limits` field to explain which constraint was hit and what the user should change (photo count, photo size, reachable image URLs, one render at a time).',
  '- Report anything in `notes` even on success: those are photos that were skipped or a WhatsApp delivery that did not go through.',
  '- If the user asks what the video generator accepts, answer from the limits reported by the video tools rather than guessing.',
  '',
